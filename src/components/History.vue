    ...
    <div class="hist">
        <div class="container">
            <div class="input-group">
                <input id="input_rid" aria-label="round_id" type="number" class="form-control" placeholder="請輸入局號"/>
                <div class="input-group-append">
                    <button @click="query()" class="btn btn-outline-secondary" type="button">Button</button>
                </div>
            </div>
        </div>
        <div id="view">
...
export default {
    ...
    methods: {
        getString(data) {
            return ['\u{2660}', '\u{2665}', '\u{2663}', '\u{2666}'][data.suit - 1] + VBTCardSymbolString[data.point];
        },
        query() {
            let rid = document.getElementById("input_rid").value ;
            // console.log(rid);
            fetch('http://localhost:55550/queryHistory?rid=' + rid, {mode: 'cors'})
                .then(res => res.json())
                .then(jsonData => {
            ...
            hists.forEach(h => {
                let str = h.map(element => {
                    ...
                    if (info && info.playCards) {
                        ...
                        return `Seat[${element.seatID}]: ${cards.map(c => this.getString(c)).join('')}`;
                    } else {
